...
class UmlsTagger {
  ...
  def buildIndex(inputFile: File, 
    ...
    //val iwconf = new IndexWriterConfig(Version.LUCENE_6_3_0, analyzer)
    ...
    Source.fromFile(inputFile)
      ...
      //println(ft.indexOptions())
      
      //ft.setIndexOptions()
      fdoc.add(new StoredField("id", fid.toString, ft))
      fdoc.add(new StoredField("cui", cui, ft))
      fdoc.add(new StoredField("str", str, ft))
      fdoc.add(new StoredField("str_norm", strNorm, ft))
      fdoc.add(new StoredField("str_sorted", strSorted, ft))
      fdoc.add(new StoredField("str_stemmed", strStemmed, ft))
  ...
  def getAnalyzer(): Analyzer = {
    //new StandardAnalyzer(Version.LUCENE_6_3_0)
